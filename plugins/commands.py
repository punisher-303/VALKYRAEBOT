...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [
            [           
            InlineKeyboardButton('📢 Uᴩᴅᴀᴛᴇꜱ 📢', url='https://t.me/{SUPPORT_CHAT}')
            ],
            [
        ...
        await message.reply(START_TXT.format(user=message.from_user.mention if message.from_user else message.chat.title, bot=client.mention), disable_web_page_preview=True, reply_markup=reply_markup)                    
    ...
    if len(message.command) != 2:
        buttons = [[
            ...
            InlineKeyboardButton('Cʜᴀɴɴᴇʟ 🔈', url='https://t.me/valkyraemovies')
            ],[      
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        ...
        await asyncio.sleep(2)
        reply_markup=InlineKeyboardMarkup(buttons)
        await message.reply_photo(photo=random.choice(PICS), caption=START_MESSAGE.format(user=message.from_user.mention, bot=client.mention), parse_mode=enums.ParseMode.HTML)
        return await m.delete()
